...
"""
Class of the grammer <prog>
 ::= program <decl-seq> begin <stmt-seq> end | program begin <stmt-seq> end
"""
class Program:
  scanner = None
  ds = None
  ...
  # Executing two sub-trees: <decl-seq>, <stmt-seq>
  def execute(self):
      if(self.ds != None):
          self.ds.execute()

      self.scanner.stack.append(self.scanner.map)
      self.scanner.map = {}

      if (self.ss != None):
          self.ss.execute()

  # printing two sub-trees: <decl-seq>, <stmt-seq>
  def print(self):
      print("program")
      if(self.ds != None):
          self.ds.print(1)
